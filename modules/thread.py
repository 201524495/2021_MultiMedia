...
class thr(threading.Thread):
    ...
    def run(self):
        ...
        for v in range(1, 101):
            ...
            time.sleep(random.randint(1, 20)/500)  # random numbers for each delay
        s.release()


class main(QWidget):  # make Layout
    def __init__(self):
        super().__init__()
        self.lst = []
        self.lstThr = []
    ...
    def setSlot(self):
        self.btn1.clicked.connect(self.normal)  # not Thread
        self.btn2.clicked.connect(self.thrMode)  # using Thread
        self.btn3.clicked.connect(self.reset)  # reset all progress bar
